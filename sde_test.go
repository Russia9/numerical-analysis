...
var testCases = []struct {
...
}{
	{
		...
		xChar: []float64{},
		hBase: 1,
		stop:  func(x float64, y ...float64) (half bool, stop bool) { return false, x >= 3 },
		expected: [][]numericalanalysis.Point2D{
			{{0, 0}, {1, 1}, {2, 2}, {3, 3}},
		},
	},
	{
		...
		xChar: []float64{},
		hBase: 0.1,
		stop:  func(x float64, y ...float64) (half bool, stop bool) { return false, x >= 0.2 },
		expected: [][]numericalanalysis.Point2D{
...
func TestMethodAccuracy(t *testing.T) {
	...
	start := []float64{1.0}
	hBase := 0.1
	stop := func(x float64, y ...float64) (half bool, stop bool) { return false, x >= 1.0 }
...
func TestDifferentialMethodErrors(t *testing.T) {
	t.Run("dimension_mismatch", func(t *testing.T) {
		...
		stop := func(x float64, y ...float64) (half bool, stop bool) { return false, x >= 1 }
...
func TestCharacteristicPoints(t *testing.T) {
	...
	methods := []struct {
		name string
		fn   func(numericalanalysis.FuncSystem, float64, []float64, []float64, float64, func(float64, ...float64) (bool, bool)) ([][]numericalanalysis.Point2D, error)
	}{
